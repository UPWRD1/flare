...
use crate::{
    passes::{
        frontend::typing::{self, Evidence, ItemWrapper, Typed},
...
impl LowerSolvedEv<'_> {
    ...
    fn make_vars<const N: usize>(&mut self, tys: [IRType; N]) -> [Var; N] {
        tys.map(|ty| {
    ...
    fn left_prod(&self) -> IRType {
        IRType::prod(Row::Closed(self.left.clone()))
    }

    fn right_prod(&self) -> IRType {
        IRType::prod(Row::Closed(self.right.clone()))
    }

    fn goal_prod(&self) -> IRType {
        IRType::prod(Row::Closed(self.goal.clone()))
    }

    fn left_sum(&self) -> IRType {
        IRType::sum(Row::Closed(self.left.clone()))
    }

    fn right_sum(&self) -> IRType {
        IRType::sum(Row::Closed(self.right.clone()))
    }

    fn goal_sum(&self) -> IRType {
        IRType::sum(Row::Closed(self.goal.clone()))
    }
    ...
    fn branch(&mut self) -> IR {
        ...
        let ret_ty = IRType::Var(TypeVar(0));

        let vars = self.make_vars([
            IRType::fun(left_sum.clone(), ret_ty.clone()),
            IRType::fun(right_sum.clone(), ret_ty.clone()),
            goal_sum,
        ]);
    ...
    fn left_enumerated_values(&self) -> impl Iterator<Item = (usize, IRType)> + use<> {
        self.left_indices.clone().into_iter().zip(self.left.clone())
    }

    fn right_enumerated_values(&self) -> impl Iterator<Item = (usize, IRType)> + use<> {
        self.right_indices
            .clone()
            .into_iter()
